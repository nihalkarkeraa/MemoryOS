...
const navSections = [
  {
    label: "Workspace",
    items: [
      { label: "Home", icon: Brain, to: "/home" },
      { label: "Documents", icon: FileText, to: "/documents" },
      { label: "Search", icon: Search, to: "/search" },
      { label: "Compare", icon: GitCompareArrows, to: "/compare" },
      { label: "Summaries", icon: ScrollText, to: "/summaries" },
    ],
  },
];
      ...
      >
              ...
              {section.items.map((item) => {
                ...
                if (item.soon) {
                  return (
                    <div
                      key={item.label}
                      className="group flex cursor-not-allowed items-center gap-3 rounded-lg px-2.5 py-2 text-muted-foreground/50"
        ...
        <div className="border-t border-sidebar-border p-3">
          ...
          >
            ...
            {!collapsed && <span className="text-[13.5px] font-medium">Settings</span>}
          </div>
          ...
          {!collapsed && (
            <div className="mb-1 flex items-center gap-2 rounded-lg px-2.5 py-2 text-[12px] text-muted-foreground">
              <span className="relative flex h-2 w-2">
                {backendStatus === "connected" && (
                  <span className={cn("absolute inline-flex h-full w-full animate-ping rounded-full", statusPing)} />
                )}
          ...
          {collapsed && (
            <div className="mb-1 flex justify-center py-1.5" title={statusLabel}>
              <span className="relative flex h-2 w-2">
                {backendStatus === "connected" && (
                  <span className={cn("absolute inline-flex h-full w-full animate-ping rounded-full", statusPing)} />
                )}
